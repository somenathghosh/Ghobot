/* eslint-disable quotes, new-cap, max-len */
/**
      ...
      let description = '';
			...
			ChatBot.getCapabilities(function(data) {
				...
				for (i = 0; i < descriptions.length; i++) {
					...
					//  console.log(pdesc);
					let matches = pdesc.match(/<span class=['"]phraseHighlight["']>['"](.+?)['"]<\/span>/gi);
					// console.log(matches);
					if (matches !== null) {
						// console.log(matches);
	...
	function playConversation(state, pauseLength, callback) {
		...
		setTimeout(function() {
			...
			if (state.start < state.currentInput.length) {
				...
				let _message = state.currentInput;
	...
	return {
		...
		init: function(options) {
			...
			$(inputs).keyup(function(e) {
				if (e.keyCode === 13) {
					...
					if (message !== '') {
						let _message = 'Received: ' + message + '  Entry: ' + lastBotspeak;
		...
		addChatEntry: function addChatEntry(text, suggestion, origin) {
			...
			if (suggestion && suggestion instanceof Array) {
				$('#suggestionsContainer').html('');
				for (let e = 0; e < suggestion.length; e++) {
			...
			// let date = new Date();
			// let currentTime = '<span>' + date.getHours() + ':' + date.getMinutes() + '</span>';

			let containerDiv = $('<div/>');
			let speakingDiv = $('<div/>').addClass((origin === 'bot' ? "icon-bot" : "icon-user"));
			let entryDiv = $('<div class="chatBotChatEntry ' + origin + (origin === 'bot' ? " left-arrow" : " right-arrow") + '" style="' + (origin === 'bot' ? "float:left" : "float:right") +'"></div>');
			let chatText = '<div class="chatText">' + text + '</div>';
